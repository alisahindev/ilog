...
export function maskSensitiveData(data: any, options: SensitiveDataMaskingOptions): any {
...
function maskValue(value: any, options: SensitiveDataMaskingOptions): string {
  ...
  const { maskCharacter = '*', preserveLength = true, showFirst = 0, showLast = 0 } = options;
